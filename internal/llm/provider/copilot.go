...
import (
	...
	toolsPkg "github.com/mohammadtihame/maebrcode/internal/llm/tools"
	"github.com/mohammadtihame/maebrcode/internal/logging"
	"github.com/mohammadtihame/maebrcode/internal/message"
...
func (c *copilotClient) exchangeGitHubToken(githubToken string) (string, error) {
	...
	req.Header.Set("User-Agent", "MaebrCode/1.0")
...
func newCopilotClient(opts providerClientOptions) CopilotClient {
	...
	if copilotOpts.bearerToken != "" {
		...
		if githubToken == "" {
			logging.Error("GitHub token is required for Copilot provider. Set GITHUB_TOKEN environment variable, configure it in maebrcode.json, or ensure GitHub CLI/Copilot is properly authenticated.")
	...
	openaiClientOptions = append(openaiClientOptions,
		option.WithHeader("Editor-Version", "MaebrCode/1.0"),
		option.WithHeader("Editor-Plugin-Version", "MaebrCode/1.0"),
